...
public class CourseScoreDAO {
	...
	public List<CourseScore> getListCourseScore(int id){
		try {
			List<CourseScore> result = new ArrayList<>();	
			String query = id == 0 ?  "SELECT * FROM CourseStudent" :  "SELECT * FROM CourseStudent WHERE StudentId=" + String.valueOf(id);
	...
	public boolean deleteCourseScoreByStudentID(CourseScore courseScore) {
		try {
